/**
 * ═══════════════════════════════════════════════════════════════════
 * FURRY ESCAPADES: OUTSMART THE VET  ·  script.js (v13)
 *
 * CAMBIOS vs v12:
 *  ✅ §3A  Menu BGM: loop profundo de baja clave (sine+triangle pad)
 *  ✅ §3B  CharSelect BGM: nuevo track rítmico y dinámico
 *  ✅ §3C  In-game BGM: Hijaz + Darbuka más cargado, bordón Db2
 *  ✅ §3   stopAll(): detiene TODOS los nodos antes de cualquier arranque
 *  ✅ §4   Router: charselect activa nuevo BGM (startCharSelectBGM)
 *  ✅ §7A  renderBackground(): paredes perimetrales + decoraciones
 *          - Molly: parquet detallado, vetas, alfombra, cuenco, calcetines
 *          - Agata: suelo musgo-líqueni, topos de seta, esporas flotantes
 *          - Martín: arenas en gradiente, grietas, cactus pixelados
 *          - Michi: baldosas slate/neon, burbujas de jabón flotantes
 *  ✅ §7B  renderHidingObject(): arte de producción para cada tipo
 *  ✅ §7C  drawDecorations(): partículas ambientales animadas por nivel
 *  ✅ §7D  Exit Portal: puerta de escape dibujada en mapa (arco de luz)
 *  ✅ §8   Timer: milisegundos en pantalla (XX.XX), urgencia roja ≤5 s
 *  ✅ §9   KABOOM: 150 partículas, screen-shake 22 frames, flash blanco
 *  ✅ §12  renderGame: decoraciones + portal + chase-border neon
 *  ✅ CHARACTERS, GS, imgPath: intactos (regla de preservación)
 * ═══════════════════════════════════════════════════════════════════
 */
'use strict';
...
/* ═══════════════════════════════════════════════════════════════════
   §2  ESTADO GLOBAL  ── estructura base preservada + extensiones v13
═══════════════════════════════════════════════════════════════════ */
const GS = {
  screen:       'loading',
  char:         null,
  images:       {},
  audioCtx:     null,
  // Nodos de audio por canal — stopAll() los limpia todos antes de cualquier arranque
  bgmNode:      null,         // menú / charselect BGM (oscilador principal)
  csNodes:      [],           // charselect extra nodes
  igMusicNodes: [],           // in-game music nodes
  isPaused:     false,
  gameLoopId:   null,
  keys:         {},
  ...
  // Partículas ambientales animadas por mapa (esporas, burbujas…)
  ambient:      []
};
...
/* ═══════════════════════════════════════════════════════════════════
   §3  AUDIO ENGINE — 100% Web Audio API, sin archivos externos
   ── OVERLAP PROTECTION: stopAll() detiene TODOS los canales ──
═══════════════════════════════════════════════════════════════════ */
...
/** Detiene y limpia TODOS los nodos activos de todos los canales. */
function stopAll() {
  [GS.bgmNode].filter(Boolean).forEach(n=>{ try{n.stop();}catch(e){} });
  GS.bgmNode = null;
  [...GS.csNodes, ...GS.igMusicNodes].forEach(n=>{ try{n.stop();}catch(e){} try{n.disconnect();}catch(e){} });
  GS.csNodes = [];
  GS.igMusicNodes = [];
}
// Aliases que el router llama por nombre legible
function stopBGM()     { stopAll(); }
function stopIgMusic() { stopAll(); }

/* ─────────────────────────────────────────────────────────────────
   §3A  MAIN MENU BGM
   Pad suave de baja clave: sine + triangle en quintas.
   Sencillo y atmosférico; no compite con la UI.
───────────────────────────────────────────────────────────────── */
function startMenuBGM() {
  ...
  try {
    ...
    // Pad fundamental (sine, D3)
    const pad1 = ctx.createOscillator();
    pad1.type = 'sine'; pad1.frequency.value = 146.83;

    // Pad de quinta (triangle, A3)
    const pad2 = ctx.createOscillator();
    pad2.type = 'triangle'; pad2.frequency.value = 220.00;

    // Melodía pentatónica lenta en triángulo
    ...
    // Guardamos el nodo principal para poder pararlo
    GS.bgmNode = pad1;
    // Los demás los ponemos en csNodes para que stopAll() los limpie
...
/* ─────────────────────────────────────────────────────────────────
   §3B  CHAR SELECT BGM
   Track rítmico y entusiasta: arpeggio square + bajo de cuerdas +
   percusión hi-hat veloz a 160 BPM.
───────────────────────────────────────────────────────────────── */
function startCharSelectBGM() {
  ...
  try {
    ...
    // Arpeggio ascendente (square) sobre escala mayor de C
    ...
    // Bajo de cuerdas (sawtooth, C2+G2)
    ...
    // Kick a 160 BPM (cada beat entero)
    for(let i=0;i<90;i++){
      const kt=now+i*BEAT;
...
/* ─────────────────────────────────────────────────────────────────
   §3C  IN-GAME BGM — Darbuka árabe 145 BPM + Maqam Hijaz full
   Melodía oud (sawtooth) + contrapunto (triangle octava alta) +
   bordón Db2 + Darbuka DUM/TEK/KA + riq off-beat
───────────────────────────────────────────────────────────────── */
function startInGameMusic() {
  ...
  try {
    ...
    // Escala Maqam Hijaz — D Eb F# G A Bb C# D (tres octavas)
    ...
    const now=ctx.currentTime;
    schedMel(H1,now,0.40,'sawtooth');     // oud principal
    schedMel(H2,now+0.05,0.18,'triangle'); // shimmer octava alta
    schedMel(H0,now,0.20,'sawtooth');      // bajo de cuerdas

    // Darbuka 145 BPM — DUM / TEK / KA + RIQ
    const B=60/145;
    const BARS=220;
    ...
    // Bordón Db2 (pedal grave)
    const drone=ctx.createOscillator(), dg=ctx.createGain();
    drone.type='sawtooth'; drone.frequency.value=69.30; // Db2
...
/* ═══════════════════════════════════════════════════════════════════
   §4  ROUTER  — stopAll() antes de cada transición de audio
═══════════════════════════════════════════════════════════════════ */
function changeScreen(screenId) {
  ...
  else if(screenId==='game')       { stopAll(); startGameplay(); }
}
...
function setupGameMap() {
  ...
  GS.timer={seconds:20,msAcc:0,lastMs:0,running:false};
  ...
  // Sembrar partículas ambientales
  seedAmbient(id);
}
...
/* ── §7-AMBIENT: partículas decorativas animadas ── */
function seedAmbient(id) {
  ...
  if(id==='agata'){
    ...
    for(let i=0;i<40;i++) GS.ambient.push({
      ...
      size:2+Math.random()*3, alpha:0.4+Math.random()*0.5,
    ...
    for(let i=0;i<35;i++) GS.ambient.push({
      ...
      vx:(Math.random()-.5)*.5, vy:-0.5-Math.random()*0.8,
...
function renderBackground(ctx) {
  ...
  if(id==='molly'){
    ...
    // Tablas de parquet con vetas de madera
    const PLANK=70;
    for(let row=0;row<Math.ceil(H/PLANK);row++){
      const base=(row%2===0)?'#5d3a1a':'#6b4423';
      ctx.fillStyle=base;
      ctx.fillRect(0,row*PLANK,W,PLANK);
      // Veta oscura de tabla
      ctx.fillStyle='rgba(40,18,8,0.25)';
      ctx.fillRect(0,row*PLANK,W,3);
      ctx.fillRect(0,row*PLANK+PLANK-3,W,3);
    }
    // Juntas verticales con offset por fila
    ctx.strokeStyle='rgba(35,15,5,0.35)'; ctx.lineWidth=2;
    for(let row=0;row<Math.ceil(H/PLANK);row++){
      const off=(row%2)*90;
    ...
    // Vetas de madera irregulares
    ctx.strokeStyle='rgba(90,55,25,0.18)'; ctx.lineWidth=1;
    for(let i=0;i<30;i++){
      const wy=(i*193)%H, wlen=80+((i*47)%120);
      ctx.beginPath();
      ctx.moveTo((i*311)%W,wy);
      ctx.bezierCurveTo((i*311)%W+wlen*.3,wy+5,(i*311)%W+wlen*.7,wy-4,(i*311)%W+wlen,wy+2);
      ctx.stroke();
    }

    // Paredes (borde interior)
    ctx.fillStyle='#e8d5c4'; ctx.fillRect(0,0,W,45);       // Norte
    ctx.fillStyle='#ddc9b5'; ctx.fillRect(0,H-45,W,45);    // Sur
    ctx.fillStyle='#e4d0bf'; ctx.fillRect(0,0,45,H);       // Oeste
    ctx.fillStyle='#dac6b4'; ctx.fillRect(W-45,0,45,H);    // Este
    // Zócalo
    ctx.fillStyle='#8b6240'; ctx.fillRect(0,40,W,10); ctx.fillRect(0,H-50,W,10);
    ctx.fillRect(40,0,10,H); ctx.fillRect(W-50,0,10,H);

    // Alfombra central
    ctx.fillStyle='#4a148c'; ctx.fillRect(280,320,760,440);
    ctx.fillStyle='#6a1cb7'; ctx.fillRect(296,336,728,408);
    ctx.strokeStyle='#9c4dcc'; ctx.lineWidth=5; ctx.strokeRect(312,352,696,376);
    // Patrón de diamantes en la alfombra
    ctx.strokeStyle='rgba(156,77,204,0.4)'; ctx.lineWidth=1;
    for(let i=1;i<8;i++){
      const cx2=312+i*(696/8), cy2=352+376/2;
      ctx.beginPath(); ctx.moveTo(cx2,352); ctx.lineTo(cx2-30,cy2); ctx.lineTo(cx2,352+376); ctx.lineTo(cx2+30,cy2); ctx.closePath(); ctx.stroke();
    }

    // Detalles decorativos no colisionables
    // Cuenco de comida
    ctx.fillStyle='#b71c1c'; ctx.beginPath(); ctx.ellipse(520,950,28,16,0,0,Math.PI*2); ctx.fill();
    ctx.fillStyle='#ef9a9a'; ctx.beginPath(); ctx.ellipse(508,946,12,8,0,0,Math.PI); ctx.fill();
    // Calcetines en el suelo (rectángulos curvilíneos)
    [[350,540,35,18,'#e91e63'],[760,620,38,18,'#2196F3'],[1100,700,34,17,'#ff5722']].forEach(([sx,sy,sw,sh,sc])=>{
      ctx.save(); ctx.translate(sx,sy); ctx.rotate(-.2);
      ctx.fillStyle=sc; ctx.beginPath(); ctx.roundRect(-sw/2,-sh/2,sw,sh,8); ctx.fill();
      ctx.restore();
    });
  ...
  /* ══════════ AGATA: THE FOREST ══════════ */
    ...
    // Capas de variación de color orgánico
    for(let i=0;i<60;i++){
      const gx=(i*347)%W, gy=(i*229)%H;
      const hue=110+((i*17)%40);
      ctx.fillStyle=`hsla(${hue},55%,${18+((i*7)%15)}%,0.7)`;
      ctx.fillRect(gx,gy,50+(i%6)*22,32+(i%5)*14);
    }

    // Textura de hierba: pequeños trazos verticales
    ...
    // Paredes de árbol boscoso (borde)
    ctx.fillStyle='#022c22'; ctx.fillRect(0,0,W,40); ctx.fillRect(0,H-40,W,40);
    ctx.fillRect(0,0,40,H); ctx.fillRect(W-40,0,40,H);
    // Enredaderas en el borde
    ctx.strokeStyle='#065f46'; ctx.lineWidth=3;
    for(let i=0;i<18;i++){
      const ex=(i*180)%(W-80)+40;
      ctx.beginPath(); ctx.moveTo(ex,0); ctx.quadraticCurveTo(ex+20,50,ex-10,90); ctx.stroke();
    }

    // Raíces en el suelo
    ctx.strokeStyle='rgba(62,39,35,0.45)'; ctx.lineWidth=2.5;
    for(let i=0;i<18;i++){
      const rx=(i*503)%W, ry=(i*317)%H;
      ctx.beginPath();
      ctx.moveTo(rx,ry);
      ctx.bezierCurveTo(rx+50,ry-25,rx+110,ry+35,rx+150,ry+12);
      ctx.stroke();
    }

    // Setas decorativas (no colisionables)
  ...
  /* ══════════ MARTÍN: THE DESERT ══════════ */
    ...
    // Ondas de arena (líneas suaves)
    ctx.strokeStyle='rgba(180,130,0,0.18)'; ctx.lineWidth=2;
    for(let i=0;i<24;i++){
      const wy=i*(H/24);
    ...
    // Grietas del suelo
    ctx.strokeStyle='rgba(130,90,0,0.30)'; ctx.lineWidth=1;
    for(let i=0;i<28;i++){
      const cx=(i*521)%W, cy=(i*389)%H;
      ctx.beginPath(); ctx.moveTo(cx,cy);
      ctx.lineTo(cx+22,cy+38); ctx.lineTo(cx+45,cy+12);
      ctx.stroke();
    }

    // Paredes de adobe (borde)
    ctx.fillStyle='#92400e'; ctx.fillRect(0,0,W,40); ctx.fillRect(0,H-40,W,40);
    ctx.fillRect(0,0,40,H); ctx.fillRect(W-40,0,40,H);

    // Cactus decorativos (no colisionables)
    [[120,300],[140,700],[1450,250],[1480,800],[800,1100]].forEach(([cx,cy])=>{
      ctx.fillStyle='#16a34a';
      ctx.fillRect(cx-8,cy-50,16,55);
      ctx.fillRect(cx-22,cy-30,14,10); ctx.fillRect(cx-22,cy-40,6,12); // brazo izq
      ctx.fillRect(cx+8,cy-25,14,10);  ctx.fillRect(cx+16,cy-36,6,12); // brazo der
      // Espinas
  ...
  /* ══════════ MICHI: THE BATHROOM / ALCHEMIST LAB ══════════ */
  } else {

    // Suelo base slate oscuro
    ...
    // Junta más gruesa cada 4 filas/columnas
    ctx.strokeStyle='rgba(6,182,212,0.45)'; ctx.lineWidth=2.5;
    ...
    ctx.fillStyle='#1e293b'; ctx.fillRect(0,0,W,40); ctx.fillRect(0,H-40,W,40);
    ctx.fillRect(0,0,40,H); ctx.fillRect(W-40,0,40,H);
    // Borde neon
    ctx.strokeStyle='rgba(6,182,212,0.6)'; ctx.lineWidth=3;
    ...
    for(let i=0;i<12;i++){
      ...
      ctx.beginPath();
      ctx.ellipse((i*370)%W,(i*270)%H,30+(i%4)*12,18+(i%3)*8,0,0,Math.PI*2);
      ctx.fill();
    }
  }
}
...
function renderHidingObject(ctx, obj) {
  ...
  switch(type){
    ...
    case 'bed': {
      ...
      // Sábana violeta
      ctx.fillStyle='#ede7f6'; ctx.fillRect(x,y,w,h*0.78);
      // Colcha con pliegues
      ctx.fillStyle='#7c4dff'; ctx.fillRect(x+8,y,w-16,h*0.72);
      // Pliegues
      ctx.strokeStyle='rgba(124,77,255,0.4)'; ctx.lineWidth=2;
      for(let i=1;i<5;i++){
        ctx.beginPath(); ctx.moveTo(x+i*(w/5),y+10); ctx.lineTo(x+i*(w/5),y+h*0.72-10); ctx.stroke();
      }
      // Cabecero
      ctx.fillStyle=color;
      ...
      // Almohadas
      ctx.fillStyle='#fff';
      ctx.beginPath(); ctx.roundRect(x+12,y+h*0.24,w*0.36,h*0.36,6); ctx.fill();
      ctx.beginPath(); ctx.roundRect(x+w*0.54,y+h*0.24,w*0.34,h*0.36,6); ctx.fill();
      // Punta del dobladillo
      ctx.fillStyle='#b39ddb'; ctx.fillRect(x,y+h*0.68,w,h*0.06);
    ...
    case 'couch': {
      ...
      ctx.fillRect(x+10,y+h-4,12,14); ctx.fillRect(x+w-22,y+h-4,12,14);
      // Cuerpo sofá en L (forma más elaborada)
      ctx.fillStyle='#5e35b1'; ctx.fillRect(x,y,w,h);
      // Respaldo
      ctx.fillStyle='#4527a0'; ctx.fillRect(x,y,w,h*0.38);
      // Asiento
      ctx.fillStyle='#7e57c2'; ctx.fillRect(x,y+h*0.38,w,h*0.62);
      // Cojines individuales con costuras
      const cushW=w*0.28, cushH=h*0.52;
      [0,1,2].forEach(i=>{
        const cx2=x+10+i*(cushW+8);
        ctx.fillStyle=i%2===0?'#9c27b0':'#ab47bc';
        ctx.beginPath(); ctx.roundRect(cx2,y+h*0.40,cushW,cushH,5); ctx.fill();
        // Botón de costura
        ctx.fillStyle='#7b1fa2';
        ctx.beginPath(); ctx.arc(cx2+cushW/2,y+h*0.40+cushH/2,4,0,Math.PI*2); ctx.fill();
      });
    ...
    case 'cabinet': {
      // Sombra
      ctx.fillStyle='rgba(0,0,0,0.3)'; ctx.fillRect(x+8,y+8,w,h);
      // Cuerpo
      ctx.fillStyle='#4e342e'; ctx.fillRect(x,y,w,h);
      // Puertas con marco biselado
      ctx.fillStyle='#6d4c41';
      ctx.beginPath(); ctx.roundRect(x+6,y+8,w/2-10,h*0.85,3); ctx.fill();
      ctx.beginPath(); ctx.roundRect(x+w/2+4,y+8,w/2-10,h*0.85,3); ctx.fill();
      // Biseles interiores
      ctx.strokeStyle='rgba(161,136,127,0.4)'; ctx.lineWidth=1;
      ctx.strokeRect(x+10,y+14,w/2-18,h*0.78);
      ctx.strokeRect(x+w/2+8,y+14,w/2-18,h*0.78);
      // Pomos dorados
      ctx.fillStyle='#f9a825';
      ctx.beginPath(); ctx.arc(x+w/2-10,y+h/2,7,0,Math.PI*2); ctx.fill();
      ctx.beginPath(); ctx.arc(x+w/2+10,y+h/2,7,0,Math.PI*2); ctx.fill();
      ctx.strokeStyle='#e65100'; ctx.lineWidth=1.5;
      ctx.beginPath(); ctx.arc(x+w/2-10,y+h/2,7,0,Math.PI*2); ctx.stroke();
      ctx.beginPath(); ctx.arc(x+w/2+10,y+h/2,7,0,Math.PI*2); ctx.stroke();
      // Zócalo y tapa
    ...
    case 'trunk': {
      // Raíces
      ctx.strokeStyle='rgba(62,39,35,0.6)'; ctx.lineWidth=4;
      [[-25,h*.85],[w+15,h*.7],[-18,h*.55]].forEach(([rx,ry])=>{
        ctx.beginPath();
        ctx.moveTo(x+w/2,y+ry);
        ctx.quadraticCurveTo(x+w/2+rx*.45,y+ry+22,x+rx,y+h+12);
        ctx.stroke();
      });
      ...
      // Textura de corteza: surcos verticales
      ctx.strokeStyle='rgba(30,15,10,0.4)'; ctx.lineWidth=2;
      for(let lx=x+10;lx<x+w-5;lx+=12){
        ctx.beginPath(); ctx.moveTo(lx,y+15);
        ctx.bezierCurveTo(lx+4,y+h/3,lx-4,y*2/3,lx+2,y+h-15);
        ctx.stroke();
      }
      // Anillos de edad
      ctx.strokeStyle='rgba(90,50,30,0.5)'; ctx.lineWidth=1.5;
      for(let ay=y+30;ay<y+h-20;ay+=42){
        ctx.beginPath(); ctx.moveTo(x+5,ay); ctx.lineTo(x+w-5,ay); ctx.stroke();
      }
    ...
    case 'bush': {
      ...
      ctx.ellipse(x+w/2+8,y+h*0.65+8,w*.52,h*.45,0,0,Math.PI*2); ctx.fill();
      // Capas de arbusto
      [{rx:.52,ry:.45,cy:.62,col:'#1b5e20'},
       {rx:.38,ry:.42,cy:.42,dx:-.22,col:'#2e7d32'},
       {rx:.35,ry:.40,cy:.38,dx:.22,col:'#388e3c'},
       {rx:.28,ry:.34,cy:.22,col:'#43a047'}
      ].forEach(({rx,ry,cy,dx=0,col})=>{
        ctx.fillStyle=col;
        ctx.beginPath();
        ctx.ellipse(x+w/2+dx*w,y+h*cy,w*rx,h*ry,0,0,Math.PI*2); ctx.fill();
      });
      // Brillo
      ctx.fillStyle='rgba(100,220,100,0.15)';
      ctx.beginPath(); ctx.ellipse(x+w*0.35,y+h*0.18,w*0.2,h*0.15,-.3,0,Math.PI*2); ctx.fill();
      break;
    }

    case 'wall': {
      // Sombra
      ctx.fillStyle='rgba(0,0,0,0.3)'; ctx.fillRect(x+6,y+6,w,h);
      // Base de muro
      ctx.fillStyle='#8d6e63'; ctx.fillRect(x,y,w,h);
      // Patrón de ladrillos detallado
      const BW=62, BH=22;
      for(let row=0;row<Math.ceil(h/BH)+1;row++){
        ...
        for(let col=-1;col<Math.ceil(w/BW)+1;col++){
          ...
          ctx.strokeStyle='rgba(62,39,35,0.5)'; ctx.lineWidth=1.5;
    ...
    case 'column': {
      // Sombra
      ctx.fillStyle='rgba(0,0,0,0.35)'; ctx.fillRect(x+7,y+7,w,h);
      ...
      ctx.strokeStyle='rgba(100,70,55,0.45)'; ctx.lineWidth=2;
      for(let lx=x+10;lx<x+w-5;lx+=10){
        ctx.beginPath(); ctx.moveTo(lx,y+22); ctx.lineTo(lx,y+h-22); ctx.stroke();
      }
      // Capitel y basa elaborados
      ctx.fillStyle='#bcaaa4'; ctx.fillRect(x-14,y,w+28,22);
      ctx.fillStyle='#d7ccc8'; ctx.fillRect(x-8,y+4,w+16,14);
      ctx.fillStyle='#bcaaa4'; ctx.fillRect(x-14,y+h-22,w+28,22);
      ctx.fillStyle='#d7ccc8'; ctx.fillRect(x-8,y+h-18,w+16,14);
    ...
    case 'well': {
      ...
      ctx.beginPath(); ctx.ellipse(x+w/2,y+h*.72,w*.54,h*.22,0,0,Math.PI*2); ctx.fill();
      // Brocal
      ctx.fillStyle='#6d4c41';
      ctx.beginPath(); ctx.arc(x+w/2,y+h/2,w*.48,0,Math.PI*2); ctx.fill();
      ctx.fillStyle='#4e342e';
      ctx.beginPath(); ctx.arc(x+w/2,y+h/2,w*.36,0,Math.PI*2); ctx.fill();
      // Agua con destellos
      const wGrad=ctx.createRadialGradient(x+w/2,y+h/2,2,x+w/2,y+h/2,w*.24);
      wGrad.addColorStop(0,'#4fc3f7'); wGrad.addColorStop(1,'#0277bd');
      ctx.fillStyle=wGrad;
      ctx.beginPath(); ctx.arc(x+w/2,y+h/2,w*.26,0,Math.PI*2); ctx.fill();
      // Travesaño
      ctx.fillStyle='#5d4037'; ctx.fillRect(x+w/2-5,y+5,10,h*.35);
      ctx.fillRect(x+w/2-w*.4,y+h*.1,w*.8,10);
      // Cuerda
      ctx.strokeStyle='#8d6e63'; ctx.lineWidth=2;
      ctx.beginPath(); ctx.moveTo(x+w/2,y+h*.1+10); ctx.lineTo(x+w/2,y+h*.45); ctx.stroke();
      ctx.strokeStyle='#3e2723'; ctx.lineWidth=5;
      ctx.beginPath(); ctx.arc(x+w/2,y+h/2,w*.48,0,Math.PI*2); ctx.stroke();
      break;
    }
    ...
    case 'dune': {
      ...
      ctx.quadraticCurveTo(x+w/2,y-30,x+w,y+h); ctx.closePath(); ctx.fill();
      ctx.fillStyle='#ca8a04';
      ctx.beginPath(); ctx.moveTo(x,y+h);
      ctx.quadraticCurveTo(x+w/2,y-15,x+w,y+h); ctx.closePath(); ctx.fill();
      ctx.fillStyle='#fef08a';
      ctx.beginPath(); ctx.moveTo(x+w*.1,y+h);
      ctx.quadraticCurveTo(x+w/2,y+5,x+w*.9,y+h); ctx.closePath(); ctx.fill();
      break;
    }
    ...
    case 'tub': {
      ...
      wg.addColorStop(0,'rgba(77,208,225,0.3)');
      wg.addColorStop(1,'rgba(77,208,225,0.65)');
      ctx.fillStyle=wg; ctx.fillRect(x+18,y+h*0.55,w-36,h*0.3);
      // Destellos en el agua
      ctx.strokeStyle='rgba(255,255,255,0.4)'; ctx.lineWidth=1.5;
      ctx.beginPath(); ctx.moveTo(x+40,y+h*0.62); ctx.lineTo(x+90,y+h*0.62); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(x+w/2,y+h*0.68); ctx.lineTo(x+w/2+50,y+h*0.68); ctx.stroke();
      // Grifo
    ...
    case 'basket': {
      ctx.fillStyle='rgba(0,0,0,0.25)'; ctx.fillRect(x+6,y+6,w,h);
      ctx.fillStyle='#4dd0e1'; ctx.fillRect(x,y,w,h);
      // Tejido mimbre
      ctx.strokeStyle='rgba(0,131,143,0.6)'; ctx.lineWidth=1.5;
      for(let cx2=x+14;cx2<x+w;cx2+=14){ ctx.beginPath(); ctx.moveTo(cx2,y); ctx.lineTo(cx2,y+h); ctx.stroke(); }
      for(let cy2=y+16;cy2<y+h;cy2+=16){ ctx.beginPath(); ctx.moveTo(x,cy2); ctx.lineTo(x+w,cy2); ctx.stroke(); }
      // Tapa con asa
    ...
    case 'shelf': {
      ...
      // Pared trasera con azulejo oscuro
      ctx.fillStyle='#0f172a'; ctx.fillRect(x+4,y+4,w-8,h-18);
      // Frascos de poción ANIMADOS
      const BCOLS=['#e040fb','#00e5ff','#69f0ae','#ff6e40','#ffea00','#f50057','#7c4dff','#00bcd4'];
      const BW2=19, BH2=h-20, GAP=26;
      let bx=x+10;
      for(let bi=0;bx+BW2<x+w-8;bi++,bx+=GAP){
        const bc=BCOLS[bi%BCOLS.length];
        // Brillo animado basado en tiempo
        const glow=0.5+0.5*Math.sin(Date.now()/400+bi);
        // Botella
        ctx.fillStyle=bc; ctx.fillRect(bx,y+6,BW2,BH2);
        // Cuello
        ctx.fillStyle=bc; ctx.fillRect(bx+4,y+2,BW2-8,6);
        // Brillo lateral
        ctx.fillStyle=`rgba(255,255,255,${0.25+glow*0.15})`;
        ctx.fillRect(bx+3,y+8,5,BH2*0.4);
        // Resplandor del líquido
        ctx.fillStyle=`rgba(${bc.slice(1).match(/../g).map(h=>parseInt(h,16)).join(',')},${0.5+glow*0.4})`;
        ctx.fillRect(bx+3,y+BH2*0.5+4,BW2-6,BH2*0.38);
        // Corcho
        ctx.fillStyle='#8d6e63'; ctx.fillRect(bx+4,y+2,BW2-8,5);
        ctx.strokeStyle=`rgba(255,255,255,${0.15+glow*0.1})`; ctx.lineWidth=1;
        ctx.strokeRect(bx,y+6,BW2,BH2);
      }
...
function renderExitPortal(ctx) {
  ...
  // Aro exterior que rota
  ctx.save(); ctx.translate(x,y); ctx.rotate(t*0.8);
  for(let i=0;i<8;i++){
    ctx.save(); ctx.rotate(i*Math.PI/4);
    ctx.fillStyle=`rgba(255,230,0,${0.3+0.1*Math.sin(t+i)})`;
    ctx.fillRect(radius-6,0,12,8);
    ctx.restore();
  }
  ctx.restore();
  ...
  aura.addColorStop(0,`rgba(255,230,0,${0.25*pulse})`);
  aura.addColorStop(1,'rgba(255,200,0,0)');
  ctx.fillStyle=aura;
  ctx.beginPath(); ctx.arc(x,y,radius*1.6,0,Math.PI*2); ctx.fill();

  // Portal principal
  const portalGrad=ctx.createRadialGradient(x,y,4,x,y,radius);
  portalGrad.addColorStop(0,`rgba(255,255,200,${pulse})`);
  portalGrad.addColorStop(0.5,`rgba(255,215,0,${0.8*pulse})`);
  portalGrad.addColorStop(1,'rgba(200,150,0,0.3)');
  ctx.fillStyle=portalGrad;
  ctx.beginPath(); ctx.arc(x,y,radius,0,Math.PI*2); ctx.fill();

  // Anillo neon
  ctx.strokeStyle=`rgba(255,230,0,${0.8*pulse})`; ctx.lineWidth=4;
...
/* ═══════════════════════════════════════════════════════════════════
   §8  COUNTDOWN TIMER — 20 s con milisegundos en pantalla
═══════════════════════════════════════════════════════════════════ */
...
function renderTimerHUD(ctx,cW){
  ...
  // Mostrar ss.cc con centésimas
  const secs=t.seconds;
  const cs=Math.floor((1000-t.msAcc)/10);
  const label=`${String(secs).padStart(2,'0')}.${String(cs).padStart(2,'0')}`;
  ...
  // Caja de fondo
  ctx.fillStyle=isUrgent?`rgba(180,0,0,${0.65*pulse})`:'rgba(0,0,0,0.6)';
  ...
  ctx.fillText(`⏱ ${label}`, cW/2, by+bH*0.68);
  ctx.shadowBlur=0;
  ctx.restore();
}

/* ═══════════════════════════════════════════════════════════════════
   §9  KABOOM — 150 partículas + screen-shake + flash
═══════════════════════════════════════════════════════════════════ */
...
function renderParticles(ctx){
  GS.particles.forEach(p=>{
    ...
    ctx.translate(p.x,p.y); ctx.rotate(p.rotate);
    ctx.fillStyle=p.color;
...
/* ═══════════════════════════════════════════════════════════════════
   §11  VET AI — patrol + chase + cono de visión
═══════════════════════════════════════════════════════════════════ */
...
function updateVetAI(){
  ...
  if(v.mode==='chase'){
    ...
    v.angle=ta;
    v.x+=Math.cos(ta)*v.speed*.55; v.y+=Math.sin(ta)*v.speed*.55;
  }
...
function renderGame(){
  ...
  if(GS.explosion.shakeFrames>0){
    const m=GS.explosion.shakeFrames*0.9;
  ...
  if(GS.explosion.active&&GS.explosion.timer<10){
    const fa=(10-GS.explosion.timer)/10*0.6;
  ...
  if(GS.vet.mode==='chase'&&!GS.player.isHidden&&!GS.gameResult){
    ...
    ctx.font=`bold ${Math.max(12,cW*.023)}px 'Press Start 2P',monospace`;
    ctx.textAlign='center';
...
function showResultOverlay(won){
  ...
  const mkBtn=(lab,ac,cb)=>{
    const b=mk('button',{fontFamily:"'Press Start 2P',monospace",fontSize:'clamp(0.4rem,1.3vw,0.65rem)',
      cursor:'pointer',padding:'0.8em 1.8em',border:`3px solid ${ac}`,background:'transparent',
      color:ac,letterSpacing:'0.1em',textTransform:'uppercase',margin:'0 8px'},lab);
  ...
  ov.appendChild(mk('div',{fontSize:'clamp(0.9rem,3vw,2rem)',color,marginBottom:'0.8rem',
    textShadow:`0 0 14px ${color},3px 3px 0 rgba(0,0,0,0.8)`,letterSpacing:'0.12em'},title));
  ov.appendChild(mk('div',{fontSize:'clamp(0.35rem,1.2vw,0.75rem)',color:'#e0e0cc',
    lineHeight:'2',marginBottom:'2rem',letterSpacing:'0.06em'},sub));
